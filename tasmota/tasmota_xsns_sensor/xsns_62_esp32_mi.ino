...
void MI32Init(void) {
  ...
  if (!MI32.mode.init) {
    ...
    NimBLEDevice::init(name);
      ...
      // const struct ble_store_key_sec key_sec = {0,0};
      // struct ble_store_value_sec value_sec;
      // ble_store_read_our_sec(&key_sec,&value_sec);
    #else
          ...
          switch(MI32.conCtx->operation){
            ...
            case 3:
              ...
              if(MI32.conCtx->hasArg1){ // characteristic selected by handle
                ...
                charvector = pSvc->getCharacteristics(); // always try to subscribe to all characteristics with the same UUID
                uint32_t position = 1;
