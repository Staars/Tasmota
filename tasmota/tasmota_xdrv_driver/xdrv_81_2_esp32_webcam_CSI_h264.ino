...
uint32_t WcSetupH264Encoder(void) {
  ...
  Wc.h264.out_buffer = (uint8_t*)esp_h264_aligned_calloc(128, 1, out_size, &out_size, ESP_H264_MEM_SPIRAM);
  ...
  // Reuse frame_buffer[0] (already allocated in SPIRAM, correct size) instead of
  // heap_caps_malloc-ing a separate ~3 MB dummy buffer
  {
    size_t frame_size = width * height * 3 / 2;
    uint8_t *probe_buf = Wc.core.frame_buffer[0];
    
    // Fill with gray/black (Y=128, U=128, V=128) and sync to SPIRAM
    memset(probe_buf, 128, frame_size);
    esp_cache_msync(probe_buf, frame_size, ESP_CACHE_MSYNC_FLAG_DIR_C2M);
    
    esp_h264_enc_in_frame_t in_f = { .raw_data = { .buffer = probe_buf, .len = frame_size } };
    esp_h264_enc_out_frame_t out_f = { .raw_data = { .buffer = Wc.h264.out_buffer, .len = Wc.h264.out_buffer_size } };
    
    // Encode one frame to extract headers
    esp_h264_err_t probe_ret = esp_h264_enc_process(Wc.h264.handle, &in_f, &out_f);
    
    if (probe_ret == ESP_H264_ERR_OK && out_f.length > 0) {
      // Scan for SPS (type 7) and PPS (type 8)
        ...
        if (p[i] == 0 && p[i+1] == 0 && p[i+2] == 0 && p[i+3] == 1) {
          ...
          if (type == 7) { // SPS
            ...
            if (sps_length > 0 && sps_length <= sizeof(Wc.h264.sps_buffer)) {
              ...
              AddLog(LOG_LEVEL_INFO, PSTR("H264: Captured SPS (%d bytes)"), sps_length);
            }
          } else if (type == 8) { // PPS
            ...
            if (pps_length > 0 && pps_length <= sizeof(Wc.h264.pps_buffer)) {
              ...
              AddLog(LOG_LEVEL_INFO, PSTR("H264: Captured PPS (%d bytes)"), pps_length);
            }
          }
        }
      }
      ...
      AddLog(LOG_LEVEL_ERROR, PSTR("H264: Probe frame encode failed (ret=0x%x, len=%d)"), probe_ret, out_f.length);
    }
    
    // Re-zero the buffer so the first real frame starts clean
    memset(probe_buf, 0, frame_size);
    esp_cache_msync(probe_buf, frame_size, ESP_CACHE_MSYNC_FLAG_DIR_C2M);
  }
  // -------------------------------------------------------------------
